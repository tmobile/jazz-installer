...
def import_item(zFile, host, org, name, importType, username, password):
    ...
    data = "--%s\r\nContent-Disposition: form-data; name=\"file\"; filename=\"%s\"\r\nContent-Type: application/x-zip-compressed\r\n\r\n%s\r\n--%s--" % (boundary, zFile, fileContent, boundary)
    req = urllib2.Request(url, data, headers)
    res = urllib2.urlopen(req)
...
def undeploy(host, org, env, flow, revision, username, password):
    ...
    req = urllib2.Request(url, 
                          headers = { 'Authorization': get_basic_auth(username, password) })
...
def deploy_api(host, org, env, apiName, revision, username, password):
    ...
    url = "%s/v1/o/%s/apis/%s/revisions/%s/deployments?action=deploy&env=%s&override=true" % (host, org, apiName, revision, env)
    req = urllib2.Request(url, 
                          headers = { 'Authorization': get_basic_auth(username, password) })
    req.get_method = lambda: 'POST'
    res = urllib2.urlopen(req)
