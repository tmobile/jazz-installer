...
if (len(sys.argv) != 3):
    print(
        "Error - Please provide Argument1(Provide Stack Name)='stack name'  \
         Argument2(Run DISABLE only and dont delete distributions)='true/false'  "
    )
...
if ((sys.argv[2].lower().__eq__("true"))
        or (sys.argv[2].lower().__eq__("false"))):
    print('')
else:
...
print("Fetching Cloud Front Distributions and Running in Directory ==" +
      fdirectory)
...
#########################################################
print("Starting disabling of CF Distributions.....of stack==" +
      sys.argv[1].lower())
    ...
    if (cfDistItem['Origins']['Items'][0]['Id'].startswith(stackName)):
        ...
        if (str(retcode).__eq__("0")):
            ...
            if (jsonCFGetConfig['DistributionConfig']['Enabled'] is True):
                ...
                print("Found Stack CF Distribution  ( " + cfDistItem['Id'] +
                ...
                retval = disableCloudFront(
                    cfDistItem['Id'], jsonCFGetConfig['ETag'], fileDistDisable)
                if (retval == 0):
...
print("Finished disabling of CF Distributions.....of stack==" +
      sys.argv[1].lower() + "\r\n\r\n")
...
print("Starting deleting of CF Distributions.....of stack==" +
      sys.argv[1].lower())
...
for cfDistItem in jsonCFDists['DistributionList']['Items']:
    if (cfDistItem['Origins']['Items'][0]['Id'].startswith(stackName)):
        ...
        if (str(retcode).__eq__("0")):
            ...
            if (jsonCFGetConfig['DistributionConfig']['Enabled'] is True):
                jsonCFGetConfig['DistributionConfig']['Enabled'] = False
                print("\t\tFirst Disabling the Cloud Front Distribution ( " +
                      cfDistItem['Id'] + " ) set enabled==" +
                      str(jsonCFGetConfig['DistributionConfig']['Enabled']))
                ...
                disableCloudFront(cfDistItem['Id'], jsonCFGetConfig['ETag'],
                                  cfDistFile)
                ...
                while (cfReadyToDelete is False):
                    ...
                    if (valuesStatus['Distribution']['Status'] == "InProgress"):
                        ...
                        time.sleep(30)
                        print(
                            "\t\t\tAwake and checking on::" + cfDistItem['Id'])
                    else:
                ...
                while (cfReadyToDelete is False):
                    ...
                    if (valuesStatus['Distribution']['Status'] == "InProgress"):
                        cfReadyToDelete = False
                        print("\t\tsleeping for 30 secs (WAITING To DELETE)::"
                              + cfDistItem['Id'])
                        time.sleep(30)
...
print("Completed deleting of CF Distributions.....of stack==" +
      sys.argv[1].lower())
