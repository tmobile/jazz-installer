...
resource "null_resource" "postJenkinsConfiguration" {
  ...
  provisioner "local-exec" {
    command = "${var.modifyCodebase_cmd}  ${aws_security_group.vpc_sg.id} ${element(aws_subnet.subnet_for_ecs_private.*.id, 1)} ${aws_iam_role.platform_role.arn} ${var.envPrefix} ${var.cognito_pool_username}"
  }
