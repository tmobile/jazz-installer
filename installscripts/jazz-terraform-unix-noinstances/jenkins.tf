resource "null_resource" "update_jenkins_configs" {
  ...
  provisioner "local-exec" {
    command = "${var.configureSubnet_cmd} ${var.dockerizedJenkins == 1 ? join(" ", aws_security_group.vpc_sg.*.id) : lookup(var.jenkinsservermap, "jenkins_security_group") } ${var.dockerizedJenkins == 1 ? join(",", aws_subnet.subnet_for_ecs_private.*.id) : lookup(var.jenkinsservermap, "jenkins_security_group") } ${var.envPrefix} ${var.jenkinsjsonpropsfile}"
  }
  provisioner "local-exec" {
