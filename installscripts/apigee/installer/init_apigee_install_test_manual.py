...
def test_get_current_deployed_version():
    expected = '1'
    actual = init_apigee_install.get_current_deployed_version('https://api.enterprise.apigee.com', 'tmobilea', 'sb02', 'InitializeRequest', apigeeUser, apigeePassword)
    assert(actual == expected)
    print 'Passed'

def test_is_api_deployed():
    expected = True
    actual = init_apigee_install.is_api_deployed('https://api.enterprise.apigee.com', 'tmobilea', 'sb02', 'InitializeRequest', '1', apigeeUser, apigeePassword)
    assert(actual == expected)
    print 'Passed'
...
def test_zip_bundle():
    ...
    path = "./test/%s" % name
    build = 'Build.1.2.3'
    sFiles = [ os.path.join(dir, addFile)
...
def test_import_item():
    ...
    actual = init_apigee_install.import_item(zFile, 'https://api.enterprise.apigee.com', 'tmobilea', 'DataEncryption', 'sharedflows', apigeeUser, apigeePassword)
    assert(actual == '8')
    print 'Passed'
...
def test_create_kvm():
    ...
    org = 'tmobilea'
    env = 'sb02'
    init_apigee_install.create_kvm(secretKey, reg, lambdaARN, 'https://api.enterprise.apigee.com', org, env, apigeeUser, apigeePassword)

def test_deploy():
    init_apigee_install.deploy('https://api.enterprise.apigee.com', 'tmobilea', 'sb02', 'DataEncryption', '8', apigeeUser, apigeePassword)

def test_undeploy():
    init_apigee_install.undeploy('https://api.enterprise.apigee.com', 'tmobilea', 'sb02', 'DataEncryption', '8', apigeeUser, apigeePassword)

def test_deploy_api():
    actual = init_apigee_install.deploy_api('https://api.enterprise.apigee.com', 'tmobilea', 'sb02', 'TestNumberManagement', '1', apigeeUser, apigeePassword)
    assert(actual)
    print 'Passed'
