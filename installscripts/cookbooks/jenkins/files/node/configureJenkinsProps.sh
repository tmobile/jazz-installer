...
echo "$0 $1 $2 "
CREDENTIAL_ID=`java -jar $JENKINS_CLI -s $JENKINS_URL -auth @$AUTHFILE list-credentials system::system::jenkins | grep "jenkins1"|cut -d" " -f1`
...
sed -i "s/CREDENTIAL_ID\".*.$/CREDENTIAL_ID\": \"$CREDENTIAL_ID\",/g" $JENKINS_JSON_PROPFILE
sed -i "s/AWS_CREDENTIAL_ID\".*.$/AWS_CREDENTIAL_ID\": \"$AWS_CREDENTIAL_ID\",/g" $JENKINS_JSON_PROPFILE
sed -i "s/CREDENTIAL_ID\".*.$/CREDENTIAL_ID\": \"$JENKINS_CREDENTIAL_ID\"/g" $JENKINS_JSON_PROPFILE
