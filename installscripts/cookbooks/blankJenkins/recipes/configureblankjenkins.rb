if node[:platform_family].include?("rhel")
    ...
    execute 'copyJobBuildPackWebsite' do
...
if node[:platform_family].include?("debian")
  ...
  execute 'copyJenkinsClientJar' do
    command "curl -sL http://localhost:8080/jnlpJars/jenkins-cli.jar -o ~/jenkins-cli.jar; chmod 755 /root/jenkins-cli.jar"
  end
  execute 'createJobExecUser' do
    command "sleep 30;echo 'jenkins.model.Jenkins.instance.securityRealm.createAccount(\"jobexec\", \"jenkinsadmin\")' | java -jar ~/jenkins-cli.jar -auth @/root/cookbooks/jenkins/files/default/authfile -s http://localhost:8080/ groovy ="
  end
  execute 'copyEncryptGroovyScript' do
  ...
  execute 'createcredentials-jenkins1' do
    command "sleep 30;/root/cookbooks/jenkins/files/credentials/jenkins1.sh localhost root"
  end
  execute 'createcredentials-jobexecutor' do
    command "/root/cookbooks/jenkins/files/credentials/jobexec.sh localhost root"
  end
  execute 'createcredentials-aws' do
    command "/root/cookbooks/jenkins/files/credentials/aws.sh localhost root"
  end
  execute 'createcredentials-cognitouser' do
    command "sleep 30;/root/cookbooks/jenkins/files/credentials/cognitouser.sh localhost root"
  end
  execute 'settingexecutepermissiononallservices' do
    command "chmod +x /root/cookbooks/jenkins/files/jobs/*.sh"
  end
  execute 'createJob-create-service' do
    command "/root/cookbooks/jenkins/files/jobs/job_create-service.sh localhost create-service #{node['bitbucketelb']} root"
  end
  execute 'createJob-delete-service' do
    command "/root/cookbooks/jenkins/files/jobs/job_delete-service.sh localhost delete-service #{node['bitbucketelb']} root"
  end
  execute 'createJob-job_build_pack_api' do
    command "/root/cookbooks/jenkins/files/jobs/job_build_java_api.sh localhost build_pack_api #{node['bitbucketelb']} root"
  end
  execute 'createJob-bitbucketteam_newService' do
    command "/root/cookbooks/jenkins/files/jobs/job_bitbucketteam_newService.sh localhost bitbucketteam_newService #{node['bitbucketelb']}  root"
  end
  execute 'job_build-deploy-platform-service' do
    command "/root/cookbooks/jenkins/files/jobs/job_build-deploy-platform-service.sh localhost build-deploy-platform-service  #{node['bitbucketelb']}  #{node['region']}  root"
  end
  execute 'job_cleanup_cloudfront_distributions' do
    command "/root/cookbooks/jenkins/files/jobs/job_cleanup_cloudfront_distributions.sh localhost cleanup_cloudfront_distributions  #{node['bitbucketelb']} root"
  end
  execute 'job_deploy-all-platform-services' do
    command "/root/cookbooks/jenkins/files/jobs/job_deploy-all-platform-services.sh localhost deploy-all-platform-services #{node['bitbucketelb']}  #{node['region']} root"
  end
  execute 'createJob-job-pack-lambda' do
    command "/root/cookbooks/jenkins/files/jobs/job_build_pack_lambda.sh localhost build-pack-lambda #{node['bitbucketelb']}  root"
  end
  execute 'createJob-job-build-pack-website' do
    command "/root/cookbooks/jenkins/files/jobs/job_build_pack_website.sh localhost build-pack-website #{node['bitbucketelb']}  root"
  end
  link '/usr/bin/aws-api-import' do
  ...
  execute 'configureJenkinsProperites' do
    command "/root/cookbooks/jenkins/files/node/configureJenkinsProps.sh localhost root"
  end
  execute 'configJenkinsLocConfigXml' do
  ...
  execute 'copyJobBuildPackApi' do
    command "sleep 20;/root/cookbooks/jenkins/files/jobs/copyJob.sh localhost build_pack_api build_pack_api_dev root"
  end
  execute 'copyJobBuildPackLambda' do
    command "sleep 20;/root/cookbooks/jenkins/files/jobs/copyJob.sh localhost build-pack-lambda build-pack-lambda-dev root"
  end
  execute 'copyJobBuildPackWebsite' do
    command "sleep 20;/root/cookbooks/jenkins/files/jobs/copyJob.sh localhost build-pack-website build-pack-website-dev root"
  end
end